...
A common use case during forensic investigations is to acquire Windows EVTX files from a large set of endpoints and then analyze their contents.  The Windows File Follower ingester is well suited to perform this tasks, however there are a few things to note when ingesting EVTX files and any other large volumes of static data in files.
