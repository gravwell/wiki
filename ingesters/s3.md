...
| Attach-Metadata         | NO            | false         | Setting to true will add the `bucket` and `key` of the file as [Intrinsic Enumerated Values](intrinsic_enumerated_values) |
...
| Secret                  | *YES*         |               | AWS IAM User secret to authenticate with |
| Region                  | *YES*         |               | AWS region the S3 bucket is registered in (Example: `us-east-2`) |
| Queue-URL               | *YES*         |               | The full URL of the SQS queue to monitor for S3 object notifications (Example: `https://sqs.us-east-2.amazonaws.com/123456789012/my-queue`) |
| Reader                  | NO            | line          | Specify how the data is to be interpreted in an S3 object, defaults to line delimited but `cloudtrail` is supported |
| Max-Line-Size           | NO            | 4MB           | Limit the maximum length of a single entry when using the `line` Reader |
| Source-Override         | NO            | Ingester IP   | Override the source value attached to each entry |
| Preprocessor            | NO            |               | Specify one or more preprocessors to execute on ingested data |
...
The following configuration monitors an SQS queue for S3 object notifications and ingests Cloudtrail logs as they arrive:

```
[SQS-S3-Listener "cloudtrail"]
	Tag-Name="aws-cloudtrail"
	ID="AKI..."
	Secret="SuperSecretKey..."
	Region="us-east-2"
	Queue-URL="https://sqs.us-east-2.amazonaws.com/123456789012/cloudtrail-notifications"
	Reader=cloudtrail
	File-Filters="AWSLogs/**/*.json.gz"
```

When an S3 event notification arrives on the queue, the ingester retrieves the referenced object and processes it according to the specified Reader.  This is the preferred method for ingesting Cloudtrail logs, as it eliminates active polling and ensures new objects are consumed promptly.
...
(amazon_cloudtrail_log_handling)=
...
Following the concept of least privilege,  it is recommended to create a new AWS IAM user or role for the ingester to use with only the bare minimum required permissions.   Below are some example AWS Permissions policies that can be used for this newly created user.
