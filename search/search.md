...
In the example above, "json" and "eval" are search modules. Note that many search modules can be made to operate on a subset of the overall query tags by putting an *additional* tag specification before the module invocation. This can be very useful if you need to perform some initial extractions differently:
...
In the example above, "table" is the render module; it has been told to display the contents of the Body field in a single column.
