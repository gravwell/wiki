...
The simplest workflow is to ingest logs from CloudTrail's S3 bucket using the [S3 Ingester](/ingesters/s3.md#amazon-cloudtrail-log-handling). The polling required may introduce a 5-15 minute delay on new logs landing in Gravwell once they hit CloudTrail, but only requires an identity that can query the CloudTrail S3 bucket.
