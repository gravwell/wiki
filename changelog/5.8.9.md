...
```{note}
This release contains a high priority bug fix for a security violation where secrets were not properly masked in some error messages when running scripts. The secrets are not leaked to logs or stored beyond temporary API responses; errors are wiped on the next run.
```
...
* Fixed an issue where secrets were not properly masked in some error messages when running scripts.
