...
Users can configure their own SMTP server by first visiting their [User Preferences](/gui/gui.html#user-preferences) page and then clicking the "Email Server" tab.
...
Here is an example configuration block which grants access to a few groups and enforces a sender and destination addresses:
...
* `Force-From`: Email address override for all sent mail.  Regardless of what users ask to send ass the `Force-From` address will be applied.
...
Note: Users must have a valid SMTP Email configuration set in their preferences or be allowed to use a shared Email configuration setup by an Admin.  If neither options are enabled users will be unable to send automated alert emails.
