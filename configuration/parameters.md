...
Each parameter has a default value which applies if the parameter is empty, or not specified in the configuration file.
...
Description:		Sets a unique identifier for a particular webserver. No two webservers should have the same UUID. If this parameter is not set, the webserver will generate a UUID and *write it back into the config file*; this is usually the best choice. Think twice before modifying this parameter.
...
Description:        The Disable-Access-Log parameter is used to disable the access log generated by the webserver.  The access logging infrastructure logs individual page accesses; while it is typically valuable to have these access logs to audit Gravwell access and to debug potential problems, the access logs can become large in environments with a lot of users, so it may be desirable to disable them.
...
Description:        The Control-Auth parameter specifies the shared secret token that is used to authenticate webservers to indexers and vice versa.  This token can be of arbitrary length; Gravwell recommends a high entropy token of at least 24 characters.  By default the installers will generate a random token.
...
Example:		`Tag-DB-Path=/tmp/path/to/tags.dat`  
...
Example:		`Max-Payload-Size-MB=16`  
...
Applies to:		Webserver  
Default Value:	(empty)  
Example:		`Webserver-Access-Control-Allow-Origin=google.com`  
...
Example:		`Ingester-State-Tracking-File=/mnt/storage/ingester_states.json`  
...
Applies to:		Webserver  
Default Value:	false  
Example:		`Disable-Access-Log-Self-Ingest=true`  
...
Applies to:		Webserver  
Default Value:	(empty)  
Example:		`Tag-Accelerator-Definitions=/opt/gravwell/etc/default.defs`  
...
Description:	This parameter, if set, allows all users to install unsigned kits. We strongly recommend against enabling this option.
...
Description:	This parameter allows the administrator to define a Content-Security-Policy header which will be sent with all Gravwell pages. This is an important security option and should be set for your organization based on your deployment requirements, such as requiring https-only.
...
Description:	When the Gravwell tile proxy is operating in normal mode (not disabled, `Map-Tile-Server` parameter not set), it will attempt to fetch map tiles from a Gravwell-operated server. If a request sent to that server fails, the proxy will instead fall back to openstreetmap.org servers for the duration of the cooldown. Setting this parameter to 0 disables the cooldown.
...
Description:	`Min-Length` specifies the minimum character length of passwords.
