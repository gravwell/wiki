...
Gravwell's GUI supports single sign-on using SAML. In theory, any SAML-compliant Identity Provider can be used to log in. This page describes Gravwell's SSO configuration options, then shows an example of how Gravwell can be configured to authenticate with a Windows AD FS server and other SSO providers.
...
* `Enable-Very-Large-Sessions` [default: true]: if set to false, Gravwell will store SSO session information directly in a session cookie, so that no persistent session information is retained. This can be useful if your IdP sends very small sessions, or if memory is extremely constrained on a datastore host or webserver. Note that cookie-based sessions are limited in size; if your IdP sends large sessions (for example, if a user is a member of dozens of groups) logins may enter a redirect loop before finally giving up. Generally this should not be disabled in any production environment.
